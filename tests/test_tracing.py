...
class TracingHelperTests(unittest.TestCase):
    ...
    def test_build_run_config_disabled_returns_only_recursion_limit(self):
        ...
        with self._patch_settings(settings):
            config = tracing.build_run_config(
                "react-agent-query",
                recursion_limit=32,
                route="react_agent",
        ...
        self.assertEqual(config, {"recursion_limit": 32})
    ...
    def test_build_run_config_enabled_includes_metadata(self):
        ...
        self.assertEqual(config["run_name"], "react-agent-query")
        self.assertEqual(config["tags"], ["react-agent-query"])
        self.assertEqual(config["metadata"]["route"], "react_agent")
        self.assertEqual(config["metadata"]["endpoint"], tracing.REACT_AGENT_QUERY_ENDPOINT)
        self.assertFalse(config["metadata"]["streaming"])
        self.assertEqual(config["metadata"]["collection_name"], "resume_bank")
        self.assertEqual(config["metadata"]["history_count"], 2)
        self.assertEqual(config["metadata"]["phase"], "planning")
